        ...
        <div className={cn(cardVariants({ gradient }), className)} {...props}>
          ...
          <div className="z-10 flex flex-col h-full">
            ...
            <div className="flex-grow">
              ...
              {subtitle && (
                <p className="text-foreground/90 font-medium mb-2 max-w-sm">{subtitle}</p>
              )}
              <p className="text-foreground/70 max-w-sm">{description}</p>
            </div>
            ...
            {tags && tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-background/50 px-3 py-1 text-xs font-medium text-foreground/80 backdrop-blur-sm"
                  >
                    {tag}
                  </span>
                ))}
